"""Compare saved decomposition-off/on answer runs without any service calls."""
...
def load_run(directory: Path, enabled: bool) -> dict:
    ...
    for snapshot in [info, *info.get("resumed", [])]:
        ...
        actual = config.pop("query_decomposition", False)
        if actual is not enabled:
            raise ValueError(f"{directory}: expected query_decomposition={enabled}")
...
def compare_runs(
    baseline: dict, candidate: dict, questions: list[dict], aliases: dict
) -> dict:
    if baseline["config"] != candidate["config"]:
        raise ValueError(
            "Agent settings differ beyond query_decomposition; not an isolated comparison"
        )
    sets = [
...
def main():
    ...
    report = compare_runs(
        load_run(args.baseline, False),
        load_run(args.candidate, True),
    ...
    print("metric | baseline | decomposition | delta")
    for key in report["baseline"]:
        values = [report[part][key] for part in ("baseline", "decomposition", "delta")]
        print(
            key
            + " | "
